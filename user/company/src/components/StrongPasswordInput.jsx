...
const StrongPasswordInput = ({ isRegister, register, errors }) => {
  ...
  //   setIsPasswordValid(password.length >= 8 && passwordStrength === 'strong');
  // };
        ...
        <div className="relative flex-1">
          ...
          {isRegister && isPasswordHovered && password && (
            <div className="absolute z-10 w-full bg-white shadow-md rounded-lg p-4 dark:bg-neutral-800 dark:border dark:border-neutral-700 dark:divide-neutral-700">
              ...
              <ul className="space-y-1 text-sm text-gray-500 dark:text-neutral-500">
                <li className={`hs-strong-password-active:text-teal-500 flex items-center gap-x-2 ${password.length < 8 ? 'text-red-500' : 'text-green-500'}`} data-hs-strong-password-hints-rule-text="min-length">
                  <span>{password.length < 8 ? '❌' : '✔'}</span>
                  <span>Minimum number of characters is 8.</span>
                </li>
                <li className={`hs-strong-password-active:text-teal-500 flex items-center gap-x-2 ${/(?=.*[a-z])(?=.*[A-Z])/.test(password) ? 'text-green-500' : 'text-red-500'}`} data-hs-strong-password-hints-rule-text="lowercase-uppercase">
                  <span>{/(?=.*[a-z])(?=.*[A-Z])/.test(password) ? '✔' : '❌'}</span>
                  <span>Should contain both lowercase and uppercase letters.</span>
                </li>
                <li className={`hs-strong-password-active:text-teal-500 flex items-center gap-x-2 ${/(?=.*\d)/.test(password) ? 'text-green-500' : 'text-red-500'}`} data-hs-strong-password-hints-rule-text="numbers">
                  <span>{/(?=.*\d)/.test(password) ? '✔' : '❌'}</span>
                  <span>Should contain at least one digit.</span>
                </li>
                <li className={`hs-strong-password-active:text-teal-500 flex items-center gap-x-2 ${/(?=.*[!@#$%^&*()_+}{"':;?/>.<,])/.test(password) ? 'text-green-500' : 'text-red-500'}`} data-hs-strong-password-hints-rule-text="special-characters">
          ...
          {isRegister && password && (
            <div className="mt-2 h-2 relative rounded-md overflow-hidden bg-gray-200">
              <div
                className={`absolute top-0 left-0 h-full ${passwordStrength === 'weak' ? 'bg-red-500' : passwordStrength === 'medium' ? 'bg-orange-500' : 'bg-green-500'}`}
